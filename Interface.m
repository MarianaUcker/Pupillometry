...
% Last Modified by GUIDE v2.5 31-Jan-2017 22:30:42
...
%         pause(.1);
        dname = uigetdir
          ...
          eval(sprintf('!mkdir %s\\Eye', dname));
          cam =webcam
...
imshow(I);
i=i+1;
saveas(gcf,strcat(dname,['\Eye\Eye_',num2str(i),'.jpg'])) 
 limiar = 120;
 d = I >= limiar;
...
if boundary(i,1) > Pmaior
 ...
 ar= diametro;
 set(handles.text7,'string',num2str(ar));
        fprintf(fid,'%12.8f \n',ar);
a=get(handles.togglebutton1,'Value');
end
fclose(fid);
